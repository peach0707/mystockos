...
test('v3 migration maps only exact decisions, preserves old prose and does not fabricate dated history',()=>{
 ...
 const values=new Map([[V3_KEY,raw]]);globalThis.localStorage={getItem:k=>values.get(k)??null,setItem:(k,v)=>values.set(k,v)};read();assert.equal(values.get(V3_KEY),raw);assert.equal(get().version,4);assert.ok(values.has(KEY));
