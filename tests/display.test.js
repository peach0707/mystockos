...
test('home follows the six agreed sections and stock notes stay on separate detail pages',()=>{const s=fresh(),h=homeView(data,s);let prev=-1;for(const title of ['市場環境','今日の方針','今強いテーマ','初動検知','保有銘柄','重要ニュース']){const i=h.indexOf('<h2>'+title);assert.ok(i>prev,title);prev=i;}assert.match(stocksView(data,s,'all'),/<a class="stock-row"/);assert.doesNotMatch(stocksView(data,s,'all'),/watch-note-form/);assert.match(stocksView(data,s,'watch','NVDA'),/watch-note-form/);});
