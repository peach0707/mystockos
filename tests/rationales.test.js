...
test('eight fixed tags, maximum three, rejects unknowns, duplicates and free text',()=>{
 const tags=Object.keys(RATIONALES),s=fresh();assert.equal(tags.length,8);assert.ok(validTags(tags.slice(0,3)));assert.ok(validTags([]));
 for(const bad of [tags.slice(0,4),['theme_growth','theme_growth'],['テーマ成長'],['constructor'],[null]]){assert.ok(!validTags(bad));assert.throws(()=>recordRationales(s,'MU',bad));}
 assert.throws(()=>recordRationales(s,'NVDAAAA',tags.slice(0,1)));assert.deepEqual(s.rationaleHistory,[]);
});
test('individual ON/OFF keeps accurate count, permits replacing a third tag, logs before/after',()=>{
 const s=fresh(),tags=Object.keys(RATIONALES);recordRationales(s,'MU',tags.slice(0,3),'2026-09-09T12:00:00.000Z','first');const first=structuredClone(s.rationaleHistory[0]);
 assert.match(rationaleDetail(s,'MU'),/維持中 3 \/ 3件/);assert.match(rationaleDetail(s,'MU'),/aria-pressed="false" disabled/);
 assert.throws(()=>toggleRationale(s,'MU',tags[3]));assert.equal(s.rationaleHistory.length,1);
 toggleRationale(s,'MU',tags[0]);assert.equal(s.rationales.MU.length,2);assert.deepEqual(s.rationaleHistory.at(-1).removed,[tags[0]]);
 toggleRationale(s,'MU',tags[3]);assert.equal(s.rationales.MU.length,3);assert.deepEqual(s.rationaleHistory[0],first);validate(s);
 recordRationales(s,'MU',[...s.rationales.MU].reverse());assert.equal(s.rationaleHistory.length,3);
 recordRationales(s,'MU',[]);assert.match(rationaleDetail(s,'MU'),/維持中 0 \/ 3件/);assert.equal(s.rationaleHistory.at(-1).removed.length,3);
});
...
test('v4 migration preserves decisions/history and legacy thesis text without inventing active tags',()=>{
 ...
 const values=new Map([[V4_KEY,raw]]);globalThis.localStorage={getItem:k=>values.get(k)??null,setItem:(k,v)=>values.set(k,v)};read();assert.equal(get().version,6);assert.equal(values.get(V4_KEY),raw);assert.ok(values.has(KEY));
...
test('holding form and registered stock detail expose tag controls without free prose',()=>{
 ...
 assert.match(detail,/維持中 2 \/ 3件/);assert.equal((detail.match(/data-rationale-tag=/g)||[]).length,8);assert.match(detail,/変更履歴/);assert.doesNotMatch(detail,/<textarea/);
 assert.doesNotMatch(stocksView({},s,'all','NVDAAAA'),/data-rationale-tag/);
 assert.doesNotMatch(rationaleFields(),/<textarea/);
});
test('old five-tag history survives the three-tag limit without silently choosing theses',()=>{
 const old={...fresh(),version:5};delete old.rationaleReview;const tags=Object.keys(RATIONALES).slice(0,5);old.rationales.MU=tags;
 old.rationaleHistory=[{id:'old',recorded_at:'2026-09-09T12:00:00.000Z',ticker:'MU',security_id:null,contexts:['watch'],before:[],after:[...tags],added:[...tags],removed:[],source:'manual',enum_version:1}];
