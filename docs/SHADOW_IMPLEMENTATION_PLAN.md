...
更新：独立ブランチで保存・台帳・自動採点基盤を実装済み。実装契約と稼働範囲は `shadow/README.md` を参照。以下は元の実装計画を保存したもの。

提出時の状態：計画のみ。設計方向はユーザー承認済み。このPRでは入力収集ジョブ、予測モデル、Prediction/Outcome Ledgerの実処理を実装しない。正式テーマ履歴が少ないため予測精度を主張しない。mainマージ・本番売買判断・Champion登録は行わない。
