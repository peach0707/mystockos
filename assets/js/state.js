...
export const KEY='mystockos.private.v5';
export const fresh = () => ({version:5,rationaleSchemaVersion:RATIONALE_VERSION,rationales:{},rationaleHistory:[],decisionSchemaVersion:DECISION_VERSION,decisionHistory:[],watchDecisions:{},cashFlows:[],cashFlowQuality:{status:'complete',note:''},yearBaselines:{},holdings:[],watch:['MU','TSM','AVGO','COHR','LITE'],snapshots:[],dividends:[],news:[],watchNotes:{},securities:{},policy:''});
...
export function validate(s){
 if(!s||s.version!==5||!['holdings','watch','snapshots','cashFlows','dividends','news'].every(k=>Array.isArray(s[k])&&s[k].length<=10000)||!str(s.policy))throw Error('対応する形式は version: 5 のバックアップです。');
 ...
 if(!Array.isArray(s.rationaleHistory)||s.rationaleHistory.some(r=>!r||!str(r.id,100)||!r.id||!ticker(r.ticker)||r.source!=='manual'||r.enum_version!==RATIONALE_VERSION||!str(r.recorded_at,30)||!dateValid(r.recorded_at.slice(0,10))||!/^\d{4}-\d{2}-\d{2}T.*Z$/.test(r.recorded_at)||!Number.isFinite(Date.parse(r.recorded_at))||!['before','after','added','removed'].every(k=>validTags(r[k]))||!(r.security_id===null||str(r.security_id,200))||!Array.isArray(r.contexts)||!r.contexts.length||r.contexts.some(x=>!['held','watch'].includes(x))||new Set(r.contexts).size!==r.contexts.length||JSON.stringify([...r.added].sort())!==JSON.stringify(r.after.filter(x=>!r.before.includes(x)).sort())||JSON.stringify([...r.removed].sort())!==JSON.stringify(r.before.filter(x=>!r.after.includes(x)).sort()))||new Set(s.rationaleHistory.map(r=>r.id)).size!==s.rationaleHistory.length)throw Error('投資根拠の変更履歴が不正です。');
...
export function migrate(input) {
 if(input?.version===5)return validate(structuredClone(input));
 if(input?.version===4){
 ...
 if(input?.version!==2)throw Error('対応する形式はversion 2 / 3 / 4 / 5です。');
 const next=structuredClone(input);
...
export function read(){
 ...
 try{
  ...
  if(raw!==null)state=validate(JSON.parse(raw));
   ...
   const old=localStorage.getItem(V4_KEY)||localStorage.getItem(V3_KEY)||localStorage.getItem(LEGACY_KEY);
   const next=old===null?fresh():migrate(JSON.parse(old));
   // Keep old keys intact; activate migrated data only after successful v5 persistence.
...
export const rawBackup=()=>localStorage.getItem(KEY)||localStorage.getItem(V4_KEY)||localStorage.getItem(V3_KEY)||localStorage.getItem(LEGACY_KEY)||JSON.stringify(state,null,2);
