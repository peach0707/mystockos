...
export function freshnessBar(data){
  const calendar=data.calendar?.value, basicDate=data.stocks?.value?.stocks?.[0]?.date;
  const dates=[basicDate,...Object.values(data.setups?.value?.stocks||{}).filter(s=>Number.isFinite(s.price)).map(s=>s.as_of)];
  const date=dates.filter(d=>['current','stale'].includes(dateState(d,calendar).state)).sort().at(-1)||basicDate,state=dateState(date,calendar);
  const problems=Object.values(data).filter(x=>x?.error).length;
  return `<div class="freshness ${state.state==='current'&&!problems?'fresh':'pending'}"><span class="status-dot"></span><span>米国 ${esc(date||'—')} ${['current','stale'].includes(state.state)?'終値':'日足'} <b>${problems?'一部取得できません':esc(state.label)}</b></span><button type="button" data-refresh aria-label="データを更新">↻</button></div>`;
}
export function updateDetails(data){return `<details class="update-details"><summary>データの更新状況</summary><div class="update-grid">${[['stocks','株価',data.stocks?.value?.stocks?.[0]?.date],['setups','銘柄チェック',data.setups?.value?.as_of],['themes','テーマ',data.themes?.value?.as_of],['regime','市場',data.regime?.value?.as_of]].map(([key,name,date])=>`<div><span>${name}</span><b>${esc(date||'未取得')}</b><small>${data[key]?.error?'取得失敗・保存分':dateState(date,data.calendar?.value).label}</small></div>`).join('')}</div><p>日足で確認します。株価は約30分ごと、銘柄チェックは米国営業日の翌朝7時台（日本時間、13時台に再確認）、ニュースは約3時間ごとに取得します。処理状況によって遅れる場合があります。</p><p>画面の確認：${esc(timeLabel(data.checkedAt))}（日本時間）</p></details>`;}
