import {recordRationales,toggleRationale} from './rationales.js';
...
document.addEventListener('change',event=>{const input=event.target;if(input.name==='rationaleTags'&&input.checked&&input.form.querySelectorAll('[name="rationaleTags"]:checked').length>5){input.checked=false;notice('投資根拠は最大5個まで選択できます。');}});
