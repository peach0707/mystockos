...
import {loadSymbols,symbolSourcesComplete,setQuoteCoverage,bindSymbolPickers,fromForm,manyFromForm,remember,registerWatch,verifyIncoming} from './symbols.js';
...
document.addEventListener('symbol-selected',event=>{const form=event.target.closest('#holding-form');if(!form||form.elements.holding_id.value)return;const ticker=event.detail.symbol,s=get(),h=s.holdings.find(h=>h.ticker===ticker);form.dataset.dirty='true';form.elements.decision.value=h?.decision||'unset';form.querySelectorAll('[name="rationaleTags"]').forEach(input=>input.checked=(s.rationales[ticker]||[]).includes(input.value));});
...
document.addEventListener('click',event=>{const b=event.target.closest('button');if(!b)return;
 ...
 if(b.dataset.editHolding){editHoldingKey=b.dataset.editHolding;addTicker='';navigate('#portfolio/edit');return;}
 if(b.dataset.addHolding){editHoldingKey='';addTicker=b.dataset.addHolding;navigate('#portfolio/edit');return;}
 if(b.dataset.manageHolding){navigate('#portfolio');const card=[...document.querySelectorAll('[data-holding]')].find(el=>el.dataset.holding===b.dataset.manageHolding);const details=card?.querySelector('details');if(details)details.open=true;card?.scrollIntoView({block:'center'});return;}
 if(b.dataset.removeHolding){if(confirm('この登録分だけを削除しますか？ 同じ銘柄のほかの登録分は残ります。'))run(()=>{mutate(s=>removeHolding(s,b.dataset.removeHolding));editHoldingKey='';navigate('#portfolio');});return;}
 ...
 const remove=[['removeFlow','cashFlows','id'],['removeDividend','dividends','id'],['removeNews','news','id']].find(([key])=>b.dataset[key]);
