...
document.addEventListener('click',event=>{const b=event.target.closest('button');if(!b)return;
 ...
 if(b.dataset.addWatch){const input=document.querySelector('#watch-form [data-symbol-query]');input.value=b.dataset.addWatch;input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();input.scrollIntoView({block:'center'});notice('候補から銘柄と市場を選択して追加してください。');}
});
