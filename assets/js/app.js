...
import {read,get,mutate,commit,validate,rawBackup,storageError} from './state.js';
import {loadPublic} from './data.js';
import {snapshotsCSV,parseCSV} from './accounting.js';
...
document.addEventListener('submit',event=>{event.preventDefault();const id=event.target.dataset.form||event.target.id,f=new FormData(event.target);run(()=>{
 ...
 else if(id==='snapshot-form')mutate(s=>{const x={date:text(f,'date'),assets:number(f,'assets'),netFlow:number(f,'netFlow'),basis:text(f,'basis'),status:text(f,'status'),components:Object.fromEntries(['price','fx','realized','other'].map(k=>[k,optional(f,k)])),...Object.fromEntries(['allocation','stockContributions','themeContributions'].map(k=>[k,rows(text(f,k))]))};s.snapshots=s.snapshots.filter(d=>d.date!==x.date);s.snapshots.push(x);selected=x.date;month=x.date.slice(0,7);});
 else if(id==='dividend-form')mutate(s=>s.dividends.push({id:crypto.randomUUID(),date:text(f,'date'),ticker:ticker(f),net:number(f,'net'),currency:text(f,'currency'),fx:number(f,'fx'),status:'paid'}));
...
document.addEventListener('click',event=>{const b=event.target.closest('button');if(!b)return;
 ...
 if(b.id==='csv-template'){download('mystockos-daily-template.csv','date,assets_jpy,net_flow_jpy,basis,status\n','text/csv');return;}
 const remove=[['removeHolding','holdings','ticker'],['removeDividend','dividends','id'],['removeNews','news','id']].find(([key])=>b.dataset[key]);
...
document.addEventListener('click',event=>{if(event.target.closest('a[href="#portfolio/edit"]'))editTicker='';});
document.addEventListener('change',async event=>{const input=event.target;if(!['import-private','import-csv'].includes(input.id)||!input.files?.length)return;try{const file=input.files[0];if(file.size>5_000_000)throw Error('ファイルは5MB以内にしてください。');const raw=await file.text();let next;if(input.id==='import-private'){next=validate(JSON.parse(raw));}else{next=structuredClone(get());for(const x of snapshotsCSV(raw)){const old=next.snapshots.find(y=>y.date===x.date);next.snapshots=next.snapshots.filter(y=>y.date!==x.date);next.snapshots.push(old?{...old,...x,components:old.components,allocation:old.allocation,stockContributions:old.stockContributions,themeContributions:old.themeContributions}:x);}validate(next);}if(confirm(`内容を検証しました。保有 ${next.holdings.length}銘柄・日次評価 ${next.snapshots.length}日。端末の記録に反映しますか？`)){commit(next);render();notice('読み込みが完了しました。');}}catch(e){notice(`保存していません：${e.message}`);}finally{input.value='';}});
render();refresh();
