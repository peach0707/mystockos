...
const coverage=m=>m&&Number.isInteger(m.total_n)?`${m.eligible_n} / ${m.total_n} 銘柄`:'取得待ち';
...
export function phaseDetails(m,asOf){
 return `<section class="phase-a-detail"><h2>期間別リターン</h2><p class="muted">価格リターン・配当を含まない／中核銘柄の有効値を均等平均</p><div class="phase-returns">${[[1,'1日'],[5,'5日'],[21,'1ヶ月'],[63,'3ヶ月']].map(([n,label])=>`<div><span>${label}</span>${amount(m?.['return_'+n+'d'])}<small>${coverage(m?.['return_'+n+'d'])}</small></div>`).join('')}</div><h2>出来高の状況</h2><div class="phase-volume"><span>テーマRVOL <b>${volume(m?.rvol)}</b></span><span>1.0倍超 ${m?.rvol?m.rvol_elevated_n+' / '+m.rvol.eligible_n:'—'}<small>有効 ${coverage(m?.rvol)}</small></span></div><p class="muted">中核銘柄の中央値。各銘柄は当日出来高 ÷ 前20営業日の平均。売買推奨ではありません。</p><h2>強さ順位の推移</h2><div class="phase-ranks">${[[5,'5日前'],[3,'3日前'],[1,'1日前'],[0,'現在']].map(([n,label])=>`<span><small>${label}</small><b>${Number.isInteger(m?.rank_history?.[n]?.rank)?m.rank_history[n].rank+'位':'—'}</b></span>`).join('')}</div><h2>構成銘柄を比較</h2><label class="phase-sort">並び順 <select data-phase-sort><option value="return_1d">1日リターン</option><option value="return_5d">5日リターン</option><option value="return_21d">1ヶ月リターン</option><option value="rvol">RVOL</option></select></label><div data-phase-members>${memberRows(m?.members||[])||'<p class="muted">構成銘柄データは取得待ちです。</p>'}</div><p class="muted">基準日 ${esc(asOf||'—')} · 1ヶ月=21営業日、3ヶ月=63営業日。欠損値・履歴未蓄積は「—」。</p></section>`;
}
