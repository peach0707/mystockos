...
export const validTags=(tags,limit=MAX_RATIONALES)=>Array.isArray(tags)&&tags.length<=limit&&tags.every(x=>typeof x==='string'&&Object.hasOwn(RATIONALES,x))&&new Set(tags).size===tags.length;
...
export function recordRationales(s,ticker,tags,recordedAt=new Date().toISOString(),id=crypto.randomUUID()){
 if(!validTags(tags))throw Error('投資根拠は選択肢から最大3個まで選んでください。');
 const held=s.holdings.some(h=>h.ticker===ticker),watch=s.watch.includes(ticker);
 if(!held&&!watch)throw Error('登録済みの銘柄を選んでください。');
 const before=canonical(s.rationaleReview?.[ticker]||s.rationales[ticker]||[]),after=canonical(tags);
 if(JSON.stringify(before)===JSON.stringify(after))return;
 s.rationales[ticker]=after;
 if(s.rationaleReview)delete s.rationaleReview[ticker];
